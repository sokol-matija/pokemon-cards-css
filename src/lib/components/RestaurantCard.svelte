<script>
  ...
  import { activeRestaurantCard } from "./activeRestaurantCard.js";
  ...
  // Popover logic
  $: activeUnsub = activeRestaurantCard.subscribe(val => {
    isActive = val === thisCard;
    if (!isActive) flipped = false;
  });
  
  function handleCardClick(e) {
    if (!isActive) {
      activeRestaurantCard.set(thisCard);
    } else {
      flipped = !flipped;
    }
    e.stopPropagation(); // Ensure the click doesn't propagate to parent elements
  }
  
  function handleOverlayClick(e) {
    activeRestaurantCard.set(undefined);
    e.stopPropagation(); // Prevent the click from reaching the card underneath
  }
  
  // Initialize the card binding after the component is mounted
  import { onMount } from 'svelte';
  onMount(() => {
    // Make sure the card is bound correctly for subsequent references
    if (thisCard) {
      // Ensure we have a clean start
      activeRestaurantCard.update(current => current === thisCard ? undefined : current);
    }
  });
</script>
...
{#if isActive}
  ...
  ></div>
{/if}
<div class="restaurant-card-outer {isActive ? 'active' : ''}">
  <div
    class="restaurant-card {flipped ? 'flipped' : ''}"
    bind:this={thisCard}
    tabindex="0"
    on:click={handleCardClick}
    on:keydown={(e) => (e.key === 'Enter' || e.key === ' ') && handleCardClick(e)}
...
.restaurant-card-outer.active .restaurant-card {
  width: 420px;
  max-width: 95vw;
  box-shadow: 0 16px 64px 0 #0008, 0 0 0 8px #ffb86b33;
